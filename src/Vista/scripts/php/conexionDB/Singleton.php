...
class Singleton{
    ...
    public static function generarConsultaPaciente2($paciente){

            $sql = 'SELECT * FROM pacientes WHERE ';
            
            $sql = $sql . 'id_paciente LIKE %'.$paciente->getIdPaciente().'% OR ';

            $sql = $sql . 'nombre LIKE %'.$paciente->getNombre().'% OR ';

            $sql = $sql . 'apellido_paterno LIKE %'.$paciente->getApellidoPaterno().'% OR ';

            $sql = $sql . 'apellido_materno LIKE %'.$paciente->getApellidoMaterno().'% OR ';

            $sql = $sql . 'num_telefono LIKE %'.$paciente->getNumTelefono().'% OR ';

            $sql = $sql . 'edad LIKE %'.$paciente->getEdad().'% OR ';
        
            $sql = $sql . 'sexo LIKE %'.$paciente->getSexo().'% OR ';

            $sql = $sql . 'direccion LIKE %'.$paciente->getDireccion().'%';
